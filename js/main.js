...
function genwallet(lang)
{
 /*if(keys && !confirm('Are you sure? This wallet cannot be recovered once a new wallet is generated.')) {
  ...
  address_widget2 = document.getElementById("address_widget");
  ...
  address_widget2.val = cnUtil.pubkeys_to_string(keys.spend.pub, keys.view.pub);
  $('.address_widget').val(cnUtil.pubkeys_to_string(keys.spend.pub, keys.view.pub));
  document.getElementById("address_widget2").value = cnUtil.pubkeys_to_string(keys.spend.pub, keys.view.pub);
  address_qr_widget.innerHTML = "";
  mnemonic_widget.innerHTML = mnemonic;

  // only uplexa has the URI scheme
  if (prefix_widget.value == "4") {
    qr=new QRCode(address_qr_widget, {correctLevel:QRCode.CorrectLevel.L});
    qr.makeCode("uplexa:"+keys.public_addr);
  }
  else {
    qr = null;
  }
...
function genwallet_prefix_worker()
{
  ...
  $('.address_widget').val(keys.public_addr);
  document.getElementById("address_widget2").value = keys.public_addr;
  address_qr_widget.innerHTML = "";
  mnemonic_widget.innerHTML = mnemonic;
